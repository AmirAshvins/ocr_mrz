...
class MrzParser {
  ...
  Map<String, dynamic> getProgress(OcrData ocrData) {
    return {'ocrText': ocrData.text, 'summaryString': _aggregator.getSummaryString(), 'shapedMrz': _aggregator.getShapedMrz(), 'fieldCounts': _aggregator.getProgress()};
  }
  ...
  MrzResult? parse(OcrData ocrData, {MrzValidationSettings settings = const MrzValidationSettings()}) {
    ...
    int targetLength;
    if ((firstCleanedLength - 44).abs() <= 5)
      targetLength = 44;
    else if ((firstCleanedLength - 36).abs() <= 5)
      targetLength = 36;
    else if ((firstCleanedLength - 30).abs() <= 5)
      targetLength = 30;
    else
      return null;
  ...
  List<String>? _findPotentialMrzLines(List<String> allLines) {
    final linePairs = allLines.map((line) => MapEntry(line, line.toUpperCase().replaceAll(RegExp(r'[^A-Z0-9<]'), ''))).where((pair) => pair.value.length > 20).toList();

    if (linePairs.length < 2) return null;

    for (int i = 0; i <= linePairs.length - 3; i++) {
      final group = linePairs.sublist(i, i + 3);
      final len1 = group[0].value.length;
      final len2 = group[1].value.length;
      final len3 = group[2].value.length;
